...
for (fami in 1:length(fam_names)) {
  ...
  for (ci in 1:10) {
    ...
    for (sim_i in 1:n_sim) {
      # Load data
      sim_dat <- read.csv(paste0('./results/data/sim5/sim_size:',n_sim,'_c:',(ci-1),'_fam:',fam_name,'_nR:',nR,'_df:',df,'_scale:',scale,'_binomoffset:',binom_offset,'_set:',sim_i-1,'.csv'))
      ...
      g1 <- gam(y ~ s(x0,bs='ps',m=c(2,2)) +
                    ...
                    s(x2,bs='ps',m=c(2,2)) +
                    s(x3,bs='ps',m=c(2,2)) +
                    s(x4,bs='re'),data=sim_dat,family = mod_fam,method='REML',control = control.g)
  ...
  # Store results
  write.table(res,file=paste0('./results/sim/mgcv_sim_5_size:',n_sim,'_n_c:',n_c,'_fam:',fam_name,'_nR:',nR,'_df:',df,'_scale:',scale,'_binomoffset:',binom_offset,'.csv'),
...
for (fami in 1:length(fam_names)) {
  ...
  for (ci in 1:10) {
    ...
    for (sim_i in 1:n_sim) {
      # Load data
      sim_dat <- read.csv(paste0('./results/data/sim5/sim_size:',n_sim,'_c:',(ci-1),'_fam:',fam_name,'_nR:',nR,'_df:',df,'_scale:',scale,'_set:',sim_i-1,'.csv'))
      sim_dat$x4 <- as.factor(sim_dat$x4)
      ...
      g1 <- gam(y ~ s(x0,bs='ps',m=c(2,2)) +
                    ...
                    s(x2,bs='ps',m=c(2,2)) +
                    s(x3,bs='ps',m=c(2,2)) +
                    s(x4,bs='re'),data=sim_dat,family = cox.ph(),weights=delta,method='REML',control = control.g)
  ...
  # Store results
  write.table(res,file=paste0('./results/sim/mgcv_sim_5_size:',n_sim,'_n_c:',n_c,'_fam:',fam_name,'_nR:',nR,'_df:',df,'_scale:',scale,'.csv'),
              sep=',',row.names = F,col.names = c("gam_acc"))
